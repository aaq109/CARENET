...
CARENET
@author: aaq109
Special thanks to     Yang et al. (2016). Hierarchical Attention Networks.
    See details for paper: https://www.cs.cmu.edu/%7Ediyiy/docs/naacl16.pdf
    and the Pytorch implementation by jaehunjung1 and JoungheeKim
"""
...
class HierarchialAttentionNetwork(nn.Module):
    ...
    def __init__(self, n_classes, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, para_rnn_size, word_rnn_layers,
                 sentence_rnn_layers, para_rnn_layers, word_att_size, sentence_att_size, para_att_size,
                 edl, hdf_len, onsite_len, dropout=0.5):
        ...
        self.para_attention = ParagraphAttention(vocab_size, emb_size, word_rnn_size, sentence_rnn_size, para_rnn_size,
                                                    word_rnn_layers, sentence_rnn_layers, para_rnn_layers, word_att_size,
                                                    sentence_att_size, para_att_size, hdf_len, dropout)
        # Classifier
        self.fc0 = nn.Linear((2 * para_rnn_size) + onsite_len, 50)
        #self.fc0 = nn.Linear((2 * para_rnn_size), 50)
        if edl:
            self.fc = nn.Linear(50, n_classes*2)
        else:
    ...
    def forward(self, X):
        documents, para_per_document, sentences_per_para, words_per_sentence, hdf, onsite = X[0], X[1], X[2], X[3], X[4], X[5]
        """
        documents = documents[:,0,:,:] (6,4,20)
        sentences_per_para = sentences_per_para[:,0] (6)
        words_per_sentence = words_per_sentence[:,0,:] (6,4)
        Forward propagation.
        ...
        document_embeddings, word_alphas, sentence_alphas, para_alphas = self.para_attention(documents, para_per_document, sentences_per_para,
                                                                                    words_per_sentence, hdf)  # (n_documents, 2 * sentence_rnn_size), (n_documents, max(sentences_per_document), max(words_per_sentence)), (n_documents, max(sentences_per_document))
        document_embeddings = torch.cat((document_embeddings, onsite), dim=1)
        # Classify
        scores = self.fc(self.relu(self.fc0(document_embeddings)))  # (n_documents, n_classes)

        return scores, word_alphas, sentence_alphas, para_alphas, document_embeddings


class ParagraphAttention(nn.Module):
    """
    The paragraph-level attention module.
    """

    def __init__(self, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, para_rnn_size, word_rnn_layers, sentence_rnn_layers,
                 para_rnn_layers, word_att_size, sentence_att_size, para_att_size, hdf_len, dropout):
        """
        ...
        self.sentence_attention = SentenceAttention(vocab_size, emb_size, word_rnn_size, sentence_rnn_size,
                                                    ...
                                                    sentence_att_size, hdf_len, dropout)

        # Bidirectional sentence-level RNN
        self.para_rnn = nn.GRU(2 * sentence_rnn_size + hdf_len, para_rnn_size, num_layers=para_rnn_layers,
                                   bidirectional=True, dropout=dropout, batch_first=True)

        # Sentence-level attention network
        self.para_attention = nn.Linear(2 * para_rnn_size, para_att_size)

        # Sentence context vector to take dot-product with
        self.para_context_vector = nn.Linear(para_att_size, 1,
    ...
    def forward(self, documents, para_per_document, sentences_per_para, words_per_sentence, hdf):
                # Re-arrange as sentences by removing sentence-pads (DOCUMENTS -> SENTENCES)
        packed_paras = pack_padded_sequence(documents,
                                                lengths=para_per_document.squeeze().tolist(),
                                                batch_first=True,
        ...
        packed_sentences_per_para = pack_padded_sequence(sentences_per_para,
                                                         lengths=para_per_document.squeeze().tolist(),
                                                         batch_first=True,
                                                         enforce_sorted=False)  # a PackedSequence object, 
                # Re-arrange sentence lengths in the same way (DOCUMENTS -> SENTENCES)
        packed_words_per_sentences = pack_padded_sequence(words_per_sentence,
                                                         lengths=para_per_document.squeeze().tolist(),
                                                         batch_first=True,
                                                         enforce_sorted=False)  # a PackedSequence object, 

        # Re-arrange as hdf by removing sentence-pads (DOCUMENTS -> SENTENCES)
        packed_hdf = pack_padded_sequence(hdf,
                                          lengths=para_per_document.squeeze().tolist(),
                                          batch_first=True,
                                          enforce_sorted=False)  # a PackedSequence object, where 'data' is the flattened sentences (n_sentences, word_pad_len)
        ...
        paras, word_alphas, sentence_alphas = self.sentence_attention(packed_paras.data,
                                                     packed_sentences_per_para.data,
                                                     packed_words_per_sentences.data)  # (n_sentences, 2 * word_rnn_size), (n_sentences, max(words_per_sentence))
        
        paras = self.dropout(paras)
        
        paras = torch.cat((paras, packed_hdf.data), dim=1)
        
        packed_paras, _ = self.para_rnn(PackedSequence(data=paras,
                                                               batch_sizes=packed_paras.batch_sizes,
                                                               sorted_indices=packed_paras.sorted_indices,
                                                               unsorted_indices=packed_paras.unsorted_indices))  # a PackedSequence object, where 'data' is th

        att_p = self.para_attention(packed_paras.data)  # (n_sentences, att_size)
        att_p = torch.tanh(att_p)  # (n_sentences, att_size)
        att_p = self.para_context_vector(att_p).squeeze(1)  # (n_sentences)
        
        max_value = att_p.max()  # scalar, for numerical stability during exponent calculation
        
        att_p = torch.exp(att_p - max_value)  # (n_sentences)

        att_p, _ = pad_packed_sequence(PackedSequence(data=att_p,
                                                      batch_sizes=packed_paras.batch_sizes,
                                                      sorted_indices=packed_paras.sorted_indices,
                                                      unsorted_indices=packed_paras.unsorted_indices),
                                       batch_first=True)  # (n_documents, max(sentences_per_document))

        para_alphas = att_p / torch.sum(att_p, dim=1, keepdim=True)  # (n_documents, max(sentences_per_document))

        documents, _ = pad_packed_sequence(packed_paras,
                                           batch_first=True)  # (n_documents, max(sentences_per_document), 2 * sentence_rnn_size)
        documents = documents * para_alphas.unsqueeze(
        ...
        sentence_alphas, _ = pad_packed_sequence(PackedSequence(data=sentence_alphas,
                                                            batch_sizes=packed_paras.batch_sizes,
                                                            sorted_indices=packed_paras.sorted_indices,
                                                            unsorted_indices=packed_paras.unsorted_indices),
        ...
        return documents, word_alphas, sentence_alphas, para_alphas
...
class SentenceAttention(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, word_rnn_layers, sentence_rnn_layers,
                 word_att_size, sentence_att_size, hdf_len, dropout):
        """
        :param vocab_size: number of words in the vocabulary of the model
        :param emb_size: size of word embeddings
        :param word_rnn_size: size of (bidirectional) word-level RNN
